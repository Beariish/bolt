...
static void btregex_compile(bt_Context* ctx, bt_Thread* thread)
{
    ...
    bt_return(thread, bt_make_object(bt_make_userdata(ctx, regex_type, &wrapped, sizeof(btregex_Regex))));
}
...
static void btregex_match(bt_Context* ctx, bt_Thread* thread)
{
    ...
    if (pm_match(regex->regex, bt_get_string(pattern), bt_string_length(pattern), regex->capture_groups, regex->group_count, 0)) {
        ...
        for (size_t i = 0; i < regex->group_count; i++) {
            bt_array_push(ctx, result, bt_make_object(bt_make_string_len(ctx, bt_get_string(pattern) + regex->capture_groups[i].start, regex->capture_groups[i].length)));
        }

        bt_return(thread, bt_make_object(result));
        return;
    }
...
static void bt_regex_all_iter(bt_Context* ctx, bt_Thread* thread)
{
    ...
    if (pm_match(regex->regex, str, len, regex->capture_groups, regex->group_count, &remainder)) {
        bt_Array* result = bt_make_array(ctx, regex->group_count);
        for (size_t i = 0; i < regex->group_count; i++) {
            bt_array_push(ctx, result, bt_make_object(bt_make_string_len(ctx, str + regex->capture_groups[i].start, regex->capture_groups[i].length)));
        }
...
void boltstd_open_regex(bt_Context* context)
{
    ...
    bt_regex_all_iter_fn = bt_make_object(bt_make_native(context, all_iter_sig, bt_regex_all_iter));
