...
#define BT_SPRINTF(...) sprintf(buffer, __VA_ARGS__)
	...
	if (BT_IS_NUMBER(value)) {
		...
		case BT_TYPE_BOOL:
			...
			break;
		case BT_TYPE_NULL: len = BT_SPRINTF("null");
			break;
		case BT_TYPE_ENUM: len = BT_SPRINTF("%d", (uint32_t)BT_AS_ENUM(value));
			break;
		default: {
			...
			break;
			case BT_OBJECT_TYPE_TYPE: len = BT_SPRINTF("%s", ((bt_Type*)obj)->name);
				break;
			case BT_OBJECT_TYPE_FN: len = BT_SPRINTF("<0x%llx: %s>", value, ((bt_Fn*)obj)->signature->name);
				break;
			case BT_OBJECT_TYPE_CLOSURE: len = BT_SPRINTF("<0x%llx: %s>", value, ((bt_Closure*)obj)->fn->signature->name);
				break;
			case BT_OBJECT_TYPE_NATIVE_FN: len = BT_SPRINTF("<Native(0x%llx): %s>", value, ((bt_NativeFn*)obj)->type ? ((bt_NativeFn*)obj)->type->name : "???");
				break;
			...
			break;
			case BT_OBJECT_TYPE_MODULE: len = BT_SPRINTF("<0x%llx: module>", value);
				break;
			default: len = BT_SPRINTF("<0x%llx: object>", value);
				break;
			}
		}
		}
...
bt_Table* bt_get_prototype(bt_Context* ctx, bt_Value obj)
{
	if (!bt_is_object(obj)) return NULL;

	bt_Object* as_obj = bt_object(obj);
	switch (BT_OBJECT_GET_TYPE(as_obj)) {
	case BT_OBJECT_TYPE_TABLE:
		return ((bt_Table*)as_obj)->prototype;
	case BT_OBJECT_TYPE_ARRAY:
		return bt_type_get_proto(ctx, ctx->types.array->prototype);
	case BT_OBJECT_TYPE_STRING:
		return bt_type_get_proto(ctx, ctx->types.string->prototype);
	default:
		return NULL;
	}
}
